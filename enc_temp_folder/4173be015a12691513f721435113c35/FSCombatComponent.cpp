...
void UFSCombatComponent::SetupAirAttackMotionWarp(float notifyStartTime, float notifyEndTime, float zOffset, float forwardOffset, float searchRadius, FName motionWarpingTargetName)
{
    ...
    // Capture SAFE avec weak pointers
    ...
    GetWorld()->GetTimerManager().SetTimer(
        ...
        {
            // Vérifier si les objets existent toujours
    ...
    // Timer pour désactiver Flying mode
...
void UFSCombatComponent::OnPowerLauncherAttackHit(AActor* hitEnemy)
{
    if (!hitEnemy)
        return;
